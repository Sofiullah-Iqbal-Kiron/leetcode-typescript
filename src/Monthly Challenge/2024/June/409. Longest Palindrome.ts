function longestPalindrome(given_string: string): number {
    // may be premature optimization
    if (given_string.length === 1) return 1

    const char_tracker: Set<string> = new Set()
    var counter = 0
    ...
    for (const current_char of given_string) {
        ...
        else char_tracker.add(current_char)
    }
